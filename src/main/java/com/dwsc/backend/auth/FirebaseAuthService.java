...
public class FirebaseAuthService {
    ...
    /**
     * Verifies a Firebase ID token (same contract as TRWM-backend {@code verifyIdToken}).
     *
     * @throws IllegalStateException if {@code FIREBASE_SERVICE_ACCOUNT_JSON} is not configured
        ...
        synchronized (lock) {
            ...
            if (raw == null || raw.isBlank()) {
                throw new IllegalStateException(
                        "FIREBASE_SERVICE_ACCOUNT_JSON is not set. Add the same secret as TRWM-backend to Cloud Run / local env.");
            }
            try {
                var cred =
